...
平衡二叉树（Balanced Binary Tree）简称平衡树（AVL树）:树上任一结点的左子树和右子树高度之差不超过1。注意：这里并没有说一定是二叉排序树
...
而建立“关键字”与“存储地址”的联系要通过“散列函数（哈希函数）”： Addr=H(Key)。
...
用质数取模，分布更均匀，冲突更少。但 散列函数的设计要结合实际的关键字分布特点来考虑。
...
比如：电话号码的后四位分布一般比较均匀，没什么规律，因此这几个分布更均匀的数码位可以取出来作为散列地址。
...
其中 $i=0,1,2,...,k(k\le m-1)$，p 是用来取模的质数，m 表示散列表表长；$d_i$为增量序列；i 可理解为“第 i 次发生冲突”。按照设计增量$d_i$的方法又可分为线性探测法，平方探测法，伪随机序列法。
...
哈希函数值域是 [0,p-1] ，冲突处理函数值域则是 [0,m-1] 

##### 查找操作

在查找失败的情况下我们从某一个位置出发，然后用线性探测法的规则一路往后扫，一直扫到第一个真正空缺的位置才能确认查找失败。

##### 删除操作

之前已经规定了碰到空位置即为查找失败。所以删除结点不能简单地将被删结点地空间置为空，否则将截断在它之后填入散列表的同义词结点的查找路径，可以做“删除标记”，进行逻辑删除。
...
(上链接)[https://blog.csdn.net/zimuzi2019/article/details/126311928]

这个算法的效率之所以不高，是因为线性探测法很容易造成**同义词，非同义词的“聚集（堆积）”现象**，严重影响查找效率。
...
#### 平方探测法
当 $d_i=0^2,1^2,-1^2,2^2,-2^2,...,k^2,-k^2$ 时，称为平方探测法，又称`二次探测法`。其中$k\le m/2$

平方探测法比起线性探测法不易产生“聚集（堆积）”问题。

>   取模运算结果是负数 -i 时，从散列表尾部从右向左插入第 i 个位置

>   散列表长度 m 必须是一个可以表示成 `4j+3` 的质数，才能探测到所有位置。比如 7、19，但是 8 不行。
...
$
H_i=RH_i(Key)\quad\quad\quad i=1,2,3,...,k
$
