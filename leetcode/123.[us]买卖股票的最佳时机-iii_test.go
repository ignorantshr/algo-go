/*
 * @lc app=leetcode.cn id=123 lang=golang
 *
 * [123] 买卖股票的最佳时机 III
 *
 * https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-iii/description/
 *
 * algorithms
 * Hard (59.01%)
 * Likes:    1606
 * Dislikes: 0
 * Total Accepted:    296.8K
 * Total Submissions: 495.9K
 * Testcase Example:  '[3,3,5,0,0,3,1,4]'
 *
 * 给定一个数组，它的第 i 个元素是一支给定的股票在第 i 天的价格。
 *
 * 设计一个算法来计算你所能获取的最大利润。你最多可以完成 两笔 交易。
 *
 * 注意：你不能同时参与多笔交易（你必须在再次购买前出售掉之前的股票）。
 *
 *
 *
 * 示例 1:
 *
 *
 * 输入：prices = [3,3,5,0,0,3,1,4]
 * 输出：6
 * 解释：在第 4 天（股票价格 = 0）的时候买入，在第 6 天（股票价格 = 3）的时候卖出，这笔交易所能获得利润 = 3-0 = 3 。
 * 随后，在第 7 天（股票价格 = 1）的时候买入，在第 8 天 （股票价格 = 4）的时候卖出，这笔交易所能获得利润 = 4-1 = 3 。
 *
 * 示例 2：
 *
 *
 * 输入：prices = [1,2,3,4,5]
 * 输出：4
 * 解释：在第 1 天（股票价格 = 1）的时候买入，在第 5 天 （股票价格 = 5）的时候卖出, 这笔交易所能获得利润 = 5-1 = 4
 * 。
 * 注意你不能在第 1 天和第 2 天接连购买股票，之后再将它们卖出。
 * 因为这样属于同时参与了多笔交易，你必须在再次购买前出售掉之前的股票。
 *
 *
 * 示例 3：
 *
 *
 * 输入：prices = [7,6,4,3,1]
 * 输出：0
 * 解释：在这个情况下, 没有交易完成, 所以最大利润为 0。
 *
 * 示例 4：
 *
 *
 * 输入：prices = [1]
 * 输出：0
 *
 *
 *
 *
 * 提示：
 *
 *
 * 1
 * 0
 *
 *
 */
package leetcode

import (
	"math"
	"testing"
)

// @lc code=start
func maxProfit123(prices []int) int {
	return maxProfit123Optimum(prices)
	return maxProfit123Base(prices)
}

func maxProfit123Optimum(prices []int) int {
	// 因为 k 的值比较小，所有可以将 k 的所有情况直接写出来
	size := len(prices)
	dp_i_1_0 := 0
	dp_i_1_1 := math.MinInt
	dp_i_2_0 := 0
	dp_i_2_1 := math.MinInt

	for i := 0; i < size; i++ {
		// dp[i][2][0] = max(dp[i-1][2][0], dp[i-1][2][1]+prices[i])
		// dp[i][2][1] = max(dp[i-1][2][1], dp[i-1][1][0]-prices[i])
		dp_i_2_0 = max(dp_i_2_0, dp_i_2_1+prices[i])
		dp_i_2_1 = max(dp_i_2_1, dp_i_1_0-prices[i])
		// dp[i][1][0] = max(dp[i-1][1][0], dp[i-1][1][1]+prices[i])
		// dp[i][1][1] = max(dp[i-1][1][1], dp[i-1][0][0]-prices[i])
		dp_i_1_0 = max(dp_i_1_0, dp_i_1_1+prices[i])
		dp_i_1_1 = max(dp_i_1_1, 0-prices[i])
	}

	return dp_i_2_0
}

func maxProfit123Base(prices []int) int {
	size := len(prices)
	dp := make([][][2]int, size)
	for i := 0; i < size; i++ {
		dp[i] = make([][2]int, 3)
		dp[i][0][0] = 0
		dp[i][0][1] = math.MinInt
	}

	for i := 0; i < size; i++ {
		for k := 2; k > 0; k-- {
			if i-1 == -1 {
				dp[i][k][0] = 0
				dp[i][k][1] = -prices[i]
				continue
			}
			dp[i][k][0] = max(dp[i-1][k][0], dp[i-1][k][1]+prices[i])
			dp[i][k][1] = max(dp[i-1][k][1], dp[i-1][k-1][0]-prices[i])
		}
	}

	return dp[size-1][2][0]
}

// @lc code=end

func Test_maxProfit123(t *testing.T) {
	tests := []struct {
		name   string
		prices []int
		want   int
	}{
		{"", []int{1}, 0},
		{"", []int{7, 6, 4, 3, 1}, 0},
		{"", []int{1, 2, 3, 4, 5}, 4},
		{"", []int{3, 3, 5, 0, 0, 3, 1, 4}, 6},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := maxProfit123(tt.prices); got != tt.want {
				t.Errorf("maxProfit123() = %v, want %v", got, tt.want)
			}
		})
	}
}
