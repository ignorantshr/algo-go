...
func TestNfa(t *testing.T) {
	...
	expr := `[a-zA-Z][a-zA-Z0-9_.]+@[a-zA-Z0-9]+.[a-zA-Z]{2,}`
	m := Compile(expr)

	for _, instance := range data {
		t.Run(fmt.Sprintf("Test: '%s'", instance.email), func(t *testing.T) {
			result := m.MathchAll(instance.email)
...
func TestNfa2(t *testing.T) {
	var data = []struct {
		...
		{email: "a", validity: true},
		{email: "bc", validity: true},
		{email: "", validity: false},
		{email: "ac", validity: false},
		{email: "abc", validity: false},
	}
